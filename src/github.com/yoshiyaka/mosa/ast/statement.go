...
func (i *If) String() string {
	if i.Else == nil {
		return fmt.Sprintf("if %s %s", i.Expression.String(), i.Block.String())
	} else {
		return fmt.Sprintf(
			"if %s %s else %s", i.Expression.String(), i.Block.String(),
			i.Else.String(),
		)
	}
}
...
func IfEquals(i1, i2 *If) bool {
	return ExpressionEquals(&i1.Expression, &i2.Expression) &&
